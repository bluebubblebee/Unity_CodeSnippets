    ...
    public class SpawnCubes : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            for (int i = 0; i < m_SampleCubes.Length; i++)
            {
                ...
                instanceSample.transform.position = Vector3.forward * 30.0f;
        ...
        void Update()
        {
            /*if ((m_SampleCubes == null) || (AudioPeer.m_Samples == null)) return;
            for (int i = 0; i < m_SampleCubes.Length; i++)
            {
                if (i < AudioPeer.m_Samples.Length)
                {
                    m_SampleCubes[i].transform.localScale = new Vector3(2.0f, (AudioPeer.m_Samples[i] * m_MaxScale) + 2.0f, 2.0f);
                }
